...
function AudioCallComponent() {
    ...
    const getPermissionsAndStream = async () => {
      try {
        ...
        toast({
            ...
            description: 'Microphone access is required for the audio call.',
        });
  ...
  const avatarVariants = {
    speaking: {
      scale: 1.05,
      boxShadow: "0px 0px 30px 5px rgba(72,149,239,0.4)",
      transition: { duration: 0.8, repeat: Infinity, repeatType: 'reverse' as const, ease: 'easeInOut' },
    },
    silent: { 
        scale: 1, 
        boxShadow: "0px 0px 15px 0px rgba(72,149,239,0.2)",
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-background p-4 text-white overflow-hidden">
        ...
        <Card className="w-full max-w-md bg-black/30 backdrop-blur-xl border border-blue-500/20 shadow-[0_0_50px_rgba(72,149,239,0.15)] rounded-2xl flex flex-col relative overflow-hidden p-8">
           <CardContent className="flex flex-col items-center justify-center h-full z-10">
                {selectedPersona?.imageUrl && (
                    <motion.div
                        className="relative w-48 h-48 md:w-64 md:h-64 rounded-full mb-8"
                ...
                <div className="text-center mb-8">
                     <h2 className="text-3xl font-bold text-white">{selectedPersona?.name || 'AI Wellness Session'}</h2>
                     <p className="text-gray-400 mt-2">{selectedPersona?.description}</p>
                </div>
                
                <div className="flex flex-col items-center w-full">
                    <StatusIndicator status={getStatus()} />

                    <div className="flex justify-center items-center gap-4 mt-8 p-3 bg-black/30 backdrop-blur-md border border-white/10 rounded-full">
